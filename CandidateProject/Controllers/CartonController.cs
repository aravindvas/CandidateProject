    ...
    public class CartonController : Controller
    {
        ...
        public ActionResult AddEquipment(int? id)
        {
            ...
            var equipment = db.Equipments
                ...
                .ToList();
            count = cartonCount.CartonDetails.Count;
            var car = db.Cartons
                .Where( c => c.Id == id)
                .SingleOrDefault();
            car.EquipmentCount = count;
            db.Cartons.AddOrUpdate(car);
            db.SaveChanges();
            if ( count < 10 )
        ...
        public ActionResult AddEquipmentToCarton([Bind(Include = "CartonId,EquipmentId")] AddEquipmentViewModel addEquipmentViewModel)
        {
            if (ModelState.IsValid)
            {
                var carton = db.Cartons
                    ...
                    .SingleOrDefault();
                var cartonCount = db.CartonDetails
                    .Count(c => c.Id == addEquipmentViewModel.CartonId);
                var equipment = db.Equipments
        ...
        public ActionResult RemoveEquipmentOnCarton([Bind(Include = "CartonId,EquipmentId")] RemoveEquipmentViewModel removeEquipmentViewModel)
        {
            if (ModelState.IsValid)
            {
                var eq = db.CartonDetails.Where(cd => (cd.EquipmentId == removeEquipmentViewModel.EquipmentId) && (cd.Carton.Id == removeEquipmentViewModel.CartonId)).SingleOrDefault();
                db.CartonDetails.Remove(eq);
                db.SaveChanges();
                var cartonCount = db.Cartons
                        .Include(c => c.CartonDetails)
                        .Where(c => c.Id == removeEquipmentViewModel.CartonId)
                        .SingleOrDefault();
                var count = cartonCount.CartonDetails.Count;
                var car = db.Cartons
                    .Where(c => c.Id == removeEquipmentViewModel.CartonId)
                    .SingleOrDefault();
                car.EquipmentCount = count;
        ...
        public ActionResult RemoveAllCartonEquipments(int? id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
            var eq = db.CartonDetails.Where(cd => cd.Carton.Id == id).ToList();
            foreach( CartonDetail e in eq)
                db.CartonDetails.Remove(e);
            db.SaveChanges();
            var car = db.Cartons
                .Where(c => c.Id == id)
                .SingleOrDefault();
            car.EquipmentCount = 0;
            db.Cartons.AddOrUpdate(car);
            db.SaveChanges();
